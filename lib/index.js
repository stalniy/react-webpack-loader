...
module.exports = function (source) {
  ...
  const descriptor = parse({
    source,
    compiler: options.compiler || require('react-template-compiler/web'),
    filename,
    sourceRoot,
    needMap: sourceMap
  })
  ...
  if (descriptor.script) {
    ...
    const injectHelper = templateImport.includes('import') ? '' : `&h=${hasScoped ? id : ''}`
    const request = stringifyRequest(src + query + injectHelper)
    scriptImport = (
...
/* normalize component */
import normalize from ${stringifyRequest(`!${componentNormalizerPath}`)}
var component = normalize(
  script,
  render,
  staticRenderFns,
  ...
  if (!isProduction) {
    code += `\ncomponent.__file = ${JSON.stringify(shortFilePath)}`
  }
